...
#         df = df[(df["A2"]==30)&(df["A1"]<=0)&(df["A1"]>=-10)&(df["theta"]>45)]
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','cx','cy','cz','E','E_i0','E_ip1','E_ip2','E_ip3','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])
    else:
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_twist.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cy','cz','a','b','theta','A1','A2','cx','E','E_i0','E_ip1','E_ip2','E_ip3','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(args):
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=6:
            ...
            E = 2*(Ei0 + Eip1 + Eip2 + Eip3 + Eit1 + Eit2 + Eit3 + Eit4)
            df_E.loc[idx, ['E_i0','E_ip1','E_ip2','E_ip3','E_it1','E_it2','E_it3','E_it4','E','status']] = [Ei0,Eip1,Eip2,Eip3,Eit1,Eit2,Eit3,Eit4,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_i0':0.,'E_ip1':0.,'E_ip2':0.,'E_ip3':0.,'E_it1':0.,'E_it2':0.,'E_it3':0.,'E_it4':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
