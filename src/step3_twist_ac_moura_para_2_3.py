...
from make_twist_ac_para_2 import exec_gjf
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','Rt','Rp','theta','A1','A2','phi1','E','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','E_it5','E_it6','machine_type','status','file_name'])##いじる
...
def listen(args):
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=12:##計算する層状の分子数
            ...
            Ei01=float(E_list[0]);Eip1=float(E_list[1]);Eip2=float(E_list[2]);Eit1=float(E_list[3]);Eit2=float(E_list[4]);Eit3=float(E_list[5]);Eit4=float(E_list[6]);Eit5=float(E_list[7]);Eit6=float(E_list[8]);Ei02=float(E_list[9]);Eip3=float(E_list[10]);Eip4=float(E_list[11])##ここも計算する分子数に合わせて調整
            E = (Ei01+Eip1+Eip2+Ei02+Eip3+Eip4)/2+Eit1+Eit2+Eit3+Eit4+Eit5+Eit6##隣接20分子　2パターン
            #### TODO
            df_E.loc[idx, ['E_i01','E_ip1','E_ip2','E_it1','E_it2','E_it3','E_it4','E_it5','E_it6','E_i02','E_ip3','E_ip4','E','status']] = [Ei01,Eip1,Eip2,Eit1,Eit2,Eit3,Eit4,Eit5,Eit6,Ei02,Eip3,Eip4,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_i01':0.,'E_ip1':0.,'E_ip2':0.,'E_it1':0.,'E_it2':0.,'E_it3':0.,'E_it4':0.,'E_it5':0.,'E_it6':0.,'E_i02':0.,'E_ip3':0.,'E_ip4':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
